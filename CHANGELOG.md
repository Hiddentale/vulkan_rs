...
- macOS surface creation now retrieves the `NSView`'s backing `CAMetalLayer` (via `raw-window-metal`) instead of reinterpreting the `NSView*` as a `CAMetalLayer*`, which caused MoltenVK to abort with an unrecognized-selector exception on every run

## [0.10.1] - 2026-07-09

### Fixed

- Codegen no longer misdetects pointer types in certain struct fields
- Scalar return values are propagated correctly instead of being dropped
- Duplicate bitmask flag values are deduplicated during generation
