...
fn init_vulkan(window: Window) -> VulkanState {
    ...
    // ════════════════════════════════════════════════════════════
    // Double buffering: per-frame sync objects and command buffers
    // ════════════════════════════════════════════════════════════
    ...
    println!(
        "Double buffering: {} frames in flight, ready to render!",
...
unsafe fn draw_frame(state: &mut VulkanState) {
    let d = &state.device;
    let sync = &state.frame_sync[state.current_frame];
    ...
    unsafe {
        ...
        d.reset_fences(&[sync.in_flight_fence])
            .expect("Failed to reset fence");

        // 4. Record commands
        d.reset_command_buffer(cmd, CommandBufferResetFlags::empty())
            .expect("Failed to reset command buffer");

        record_commands(d, state, cmd, image_index);
        ...
        d.queue_submit(state.graphics_queue, &[*submit_info], sync.in_flight_fence)
...
unsafe fn record_commands(
    ...
    unsafe {
        ...
        device.cmd_bind_pipeline(command_buffer, PipelineBindPoint::GRAPHICS, state.pipeline);
...
unsafe fn cleanup(state: VulkanState) {
    unsafe {
        ...
        for sync in &state.frame_sync {
            state.device.destroy_fence(sync.in_flight_fence, None);
            state.device.destroy_semaphore(sync.image_available, None);
            state.device.destroy_semaphore(sync.render_finished, None);
        }
