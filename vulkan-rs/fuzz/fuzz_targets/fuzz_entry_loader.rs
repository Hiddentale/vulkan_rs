...
unsafe extern "system" fn stub_get_instance_proc_addr(
    _instance: vulkan_rs_sys::handles::Instance,
    _name: *const std::ffi::c_char,
) -> vulkan_rs_sys::structs::PFN_vkVoidFunction {
    None
}

/// Minimal valid vkGetDeviceProcAddr: returns null for all lookups.
unsafe extern "system" fn stub_get_device_proc_addr(
    _device: vulkan_rs_sys::handles::Device,
    _name: *const std::ffi::c_char,
) -> vulkan_rs_sys::structs::PFN_vkVoidFunction {
    None
}
...
fuzz_target!(|data: &[u8]| {
    ...
    match unsafe { vulkan_rs::Entry::new(loader) } {
        Ok(entry) => {
        ...
        Err(vulkan_rs::LoadError::MissingEntryPoint) => {
            // gipa was null, which is correct when gipa_mode == 0.
            assert_eq!(data[0], 0);
        }
        Err(vulkan_rs::LoadError::Library(_)) => {
